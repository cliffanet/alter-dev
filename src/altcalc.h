...
// Особенности:
// - начало подъёма тоже профилируется (сверяется с профилем)
// - начало прыжка - всегда FF (т.е. раскрытие под бортом уже не прокатит)
// - профиль работает по усреднённым значениям за 10 измерений, а не за весь интервал
...
public:
    ...
    const bool      isinit()    const { return !_d.full(); }
    // Текущее давление
    const float     press()     const { return _d.size() > 0 ? _d.last().press : 0; }
    // Текущая высота
    const float     alt()       const { return _d.size() > 0 ? _d.last().alt : 0; }
    // Интервал времени с предыдущего измерения
    const float     tm()        const { return _d.size() > 0 ? _d.last().interval : 0; }
    // суммарный интервал времени, за которое собраны данные
    const uint32_t  interval()  const;

    // средне-арифметические: высота и скорость
    const VAvg      avg()       const { return VAvg(_alt0, _d); }
    // по аппроксимации: высота и скорость
    const VApp      app()       const { return VApp(_alt0, _d); }
...
#ifdef AC_SHORT
        // Высота - среднее за AC_SHORT циклов
        const float         altsav()    const;
#endif
