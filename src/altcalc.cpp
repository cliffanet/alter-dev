...
const double AltCalc::sqdiff() const {
    ...
    return sqrt(sq / _d.size());
}

#ifdef AC_SHORT
// Высота - среднее за AC_SHORT циклов
const float AltCalc::altsav() const {
    double alt = 0;
    for (uint8_t i = 0; i < AC_SHORT; i++)
        alt += _d[i].alt;
    return alt / AC_SHORT;
}
#endif
...
void AltSqBig::tick(const AltCalc &ac) {
    _val = ac.sqdiff();
    // большая турбуленция (высокое среднеквадратическое отклонение)
    if (!_big && (_val >= AC_JMP_SQBIG_THRESH)) {
        _big = true;
        _cnt = 0;
        _tm = 0;
    }
    else
    if (_big && (_val < AC_JMP_SQBIG_MIN)) {
        _big = false;
        _cnt = 0;
        _tm = 0;
    }
...
void AltJmp::tick(const AltCalc &ac) {
    ...
    switch (_mode) {
        ...
        case FREEFALL:
            ...
            // Для jmp только такой порядок переходов,
            // это гарантирует прибавление только одного прыга на счётчике при одном фактическом
            if (avg.speed() >= -AC_JMP_SPEED_CANOPY) {
                _c_cnt++;
                _c_tm += tint;
                if ((_c_cnt >= AC_JMP_CNP_COUNT) && (_c_cnt >= AC_JMP_CNP_TIME))
                    m = CANOPY;
            }
            else
            if (_c_cnt > 0) {
        ...
        case ACJMP_CANOPY:
            if (
                ...
                _c_cnt++;
                _c_tm += tint;
                if ((_c_cnt >= AC_JMP_GND_COUNT) && (_c_cnt >= AC_JMP_GND_TIME))
                    m = GROUND;
            }
            else
            if (_c_cnt > 0) {
